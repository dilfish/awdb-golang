...
// field is Metadata, which contains the metadata from the aw DB file.
//
...
// FromBytes takes a byte slice corresponding to a aw DB file and returns
// a Reader structure or an error.
func FromBytes(buffer []byte) (*Reader, error) {
	metadataStart := bytes.LastIndex(buffer, metadataStartMarker)

	if metadataStart == -1 {
		return nil, newInvalidDatabaseError("error opening database: invalid aw DB file")
	}
	...
	if dataSectionStart > dataSectionEnd {
		return nil, newInvalidDatabaseError("the aw DB contains invalid metadata")
	}
	d := decoder{
...
func (r *Reader) cidr(ip net.IP, prefixLength int) *net.IPNet {
	...
	// with databases currently distributed by aw as all of them
...
// value from the offset. aw DBs are highly normalized: for example in
...
func (r *Reader) resolveDataPointer(pointer uint) (uintptr, error) {
	...
	if resolved >= uintptr(len(r.buffer)) {
		return 0, newInvalidDatabaseError("the aw DB file's search tree is corrupt")
	}
	return resolved, nil
}
